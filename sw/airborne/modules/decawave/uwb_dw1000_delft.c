...
static void fill_anchor_Cust(struct DW1000 *dw) {
  ...
  for (uint8_t i = 0; i < DW1000_NB_ANCHORS; i++) {
    ...
    /*float norm = sqrtf((dw->anchors[i].distance - float_from_buf(dw->buf+1))*(dw->anchors[i].distance - float_from_buf(dw->buf+1))); norm < 2 */
...
static void send_gps_dw1000_small(struct DW1000 *dw)
{
  ...
  float z=(*pos2).z;  //This still needs to be tested. Hopefully fixes the error on takeoff

  //--Outlier rejection:
  /*float error = sqrtf((aveX[count-1]-x)*(aveX[count-1]-x)+(aveY[count-1]-y)*(aveY[count-1]-y)); error > 10*/
  float error = ((aveX[count-1]-x)*(aveX[count-1]-x)+(aveY[count-1]-y)*(aveY[count-1]-y));
  if (error > 100)
  {
    if(count == 0)
    {
